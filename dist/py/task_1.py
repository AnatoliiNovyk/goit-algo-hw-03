...
def main():
    try:
        ...
        # argparse.ArgumentParser.exit(status=..., message=...) calls sys.exit(status).
        # The message is printed by argparse.ArgumentParser.error(message).
        # This try-except block makes the handling of this specific exit explicit
        # within the main function's control flow.
        # We rely on argparse to print the error; we just ensure the script exits
        # with the appropriate status code (typically 2 for argument errors).
