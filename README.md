...
- Функція `koch_curve` рекурсивно малює один сегмент кривої Коха.
	- **Базовий випадок:** Якщо `order` (рівень рекурсії) дорівнює `0`, `turtle` просто малює пряму лінію заданої довжини. 
	- **Рекурсивний крок:** Якщо `order > 0`, лінія ділиться на три частини. Замість середньої частини малюється "зубець", що складається з двох сторін рівностороннього трикутника. Це досягається чотирма рекурсивними викликами `koch_curve` з `order - 1` та відповідними поворотами `turtle`. 

- Функція `draw_koch_snowflake` налаштовує екран та `turtle`, а потім викликає `koch_curve` тричі, повертаючи `turtle` на 120 градусів після кожного сегменту, щоб сформувати сніжинку (рівносторонній трикутник, сторони якого є кривими Коха).
